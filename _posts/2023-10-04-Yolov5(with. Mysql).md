...
date: 2023-10-04
categories: yolov5 detect.py computervision vision sensor mysql dataset data communication pymysql object detect location estimation
---

# Yolov5 Detect.py 코드 수정
- 장애물 감지 및 Unreal Engine4(UE4)에 장애물 데이터 전송을 위해 detect.py 코드 수정
- 동일 객체 구분을 위한 ID 부여 코드 생성
- 카메라의 픽셀을 활용하여 위치 추정 (픽셀 * 0264583333 = cm)
- pymysql을 사용하여 Mysql 서버에 실시간 데이터 전송, 업데이트, 삭제 기능 추가

## [전체코드](https://github.com/JeongMin-D/yolov5Test/blob/master/test4.py)
